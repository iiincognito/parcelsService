...
func (s ParcelStore) GetByClient(client int) ([]Parcel, error) {
	...
	res, err := s.db.Query("SELECT * FROM parcel WHERE client=?", sql.Named("client", client))
	if err != nil {
		return nil, err
	}
	defer res.Close()
...
func (s ParcelStore) SetStatus(number int, status string) error {
	...
	if status != ParcelStatusDelivered && status != ParcelStatusSent {
		return fmt.Errorf("invalid parcel status: %s", status)
	}
